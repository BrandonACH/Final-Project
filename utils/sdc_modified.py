...
class SDCAnalysis:
    ...
    def get_ranges_df(self, bin_size: int = 3, alpha: float = .05, min_bin=None, max_bin=None, threshold: float = .0, ts: int = 1):
    ...
    def combi_plot(self, alpha: float = .05, xlabel: str = '', ylabel: str = '', title: str = None, max_r: float = None,
        ...
        if min_lag < 0:
            mc1 = fig.add_subplot(gs[-1, 1:3], sharex=ts1)  # Sharing x-axis with ts1
            (self.sdc_df
            .loc[lambda dd: dd.p_value < alpha]
            .loc[lambda dd: (dd.lag <= max_lag) & (dd.lag >= min_lag)]
            .groupby('date_1')
